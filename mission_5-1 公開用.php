...
//あらかじめ作成しておいたtableを使う
$sql = "CREATE TABLE IF NOT EXISTS mission_丸5_01"
." ("
...
if(!empty($_POST["comment"])){
    ...
    if(!empty($_POST["edit_post"])){
        ...
        $re_date = $date."編集済み";
        $sql = 'UPDATE mission_丸5_01 SET name=:name,comment=:comment,date=:re_date,pwd=:pwd 
        WHERE id=:re_id';
    ...
    //編集でないときは、普通に入力
        ...
        $sql = $pdo -> prepare("INSERT INTO mission_丸5_01 (name, comment, date, pwd) 
...
elseif(!empty($_POST["del_num"])){
    ...
    //andで繋げれば複数の条件で絞り込める
    $sql = 'delete from mission_丸5_01 where id=:del_id and pwd=:del_pwd';
    $stmt = $pdo->prepare($sql);
...
elseif(isset($_POST["edit_num"])){
    ...
    $sql = 'SELECT * FROM mission_丸5_01 WHERE id=:edit_id and pwd=:edit_pwd';
    $stmt = $pdo->prepare($sql); 
